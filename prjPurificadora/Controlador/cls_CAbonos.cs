/*en esta clase se consultan a los clientes que han quedado a deber diner d las compras para realizar abonos*/
using System;
...
namespace prjPurificadora.Controlador
{
    class cls_CAbonos
    {
        frmAbonos frm = new frmAbonos();//instanciamos el formulario de abonos
        ...
        public void llenatabla()//aqui llenamos la consulta de la tabla deudas de la bd para ver los clientes que han quedado ha deber dinero
        {
            try
            {
                frm.tblDeudas.DataSource = mtd.mtdllenar(dts);
                ...
                if (!(frm.tblDeudas.Rows.Count >= 1))//preguntamos si hay registros, delo contrario se avisa con el menseje de que no tiene deudas
                {
                    frm.btnRegistrar.Enabled = false;
                    frm.txtAbono.Enabled = false;
                    MessageBox.Show("No hay deudas para este vendedor :)", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
